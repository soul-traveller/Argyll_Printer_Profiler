# Argyll_Printer_Profiler.command — User Guide
**Version:** 1.3.2<br>
**Platform:** macOS and Linux<br>
**Based on:** Simple script by Jintak Han (https://github.com/jintakhan/AutomatedArgyllPrinter)<br>
**Author:** Knut Larsson<br>

Argyll_Printer_Profiler.command is an interactive terminal script that automates a complete **ArgyllCMS printer profiling workflow** on macOS, Linux and Windows, from target generation to ICC installation.<br>

See full Reference and Usage Guide:<br>
https://soul-traveller.github.io/Argyll_Printer_Profiler/
---
